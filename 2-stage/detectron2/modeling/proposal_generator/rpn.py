...
class StandardRPNHead(nn.Module):
    ...
    def __init__(
        ...
        if len(conv_dims) == 1:
            ...
            self.conv_S2 = self._get_rpn_conv(cur_channels*2, out_channels)
            ...
            for k, conv_dim in enumerate(conv_dims):
                ...
                conv_S2 = self._get_rpn_conv(cur_channels*2, out_channels)
                self.conv.add_module(f"conv{k}", conv)
                self.conv_S2.add_module(f"conv{k}", conv_S2)
                cur_channels = out_channels
